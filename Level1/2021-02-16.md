...
- __2.1 스트림이란?__
    ...
    - __[Stream<Integer> 와 IntStream]__
        - 오토박싱&언박싱으로 인한 비효율을 줄이기 위해 데이터 소스의 요소를 기본형(int, long, double)으로 다루는 스르림 제공
...
- __2.2 스트림 만들기__
    ...
    - __[빈 스트림]__
        ...
        - 연산 수행 후 결과가 하나도 없다면, null이 아닌 빈 스트림 반환하자
            - ex) Stream emptyStream = Stream.empty();
...
- __2.5 스트림의 최종연산__
    ...
    - __[통계 - count(), sum(), average(), max(), min()]__
        - 기본 스트림이 아닐 경우, count() / min() / max() 만 가능
    
    - __[리듀싱 - reduce()]__
                                                                            
- __2.6 collect()__

- __2.7 Collector 구현하기__
